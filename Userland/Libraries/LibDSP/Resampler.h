...
template<size_t sinc_taps, size_t oversample>
...
template<typename SampleType, typename SincFunction>
requires(IsCallableWithArguments<SincFunction, float, float>)
class SincResampler {
public:
    ...
    size_t process(ReadonlySpan<SampleType> input, Span<SampleType> output)
    {
        ...
        while (sinc_center_index + m_sinc_taps + 1 < m_input_buffer_size) {
            ...
            m_phase = m_phase - static_cast<float>(static_cast<i64>(m_phase));
