...
NonnullOwnPtr<LoadUnsignedImmediate> parse_lui(u32 instruction)
{
    ...
    return adopt_own(*new (nothrow) LoadUnsignedImmediate(raw_parts.imm, raw_parts.rd));
}
...
{
    auto raw_parts = RawUType::parse(instruction);
    return adopt_own(*new (nothrow) AddUnsignedImmediateToProgramCounter(raw_parts.imm, raw_parts.rd));
}
...
NonnullOwnPtr<InstructionImpl> parse_misc_mem(u32 instruction)
{
    ...
    case 0b001: {
        return adopt_own(*new (nothrow) InstructionFence);
    }
    default:
        VERIFY_NOT_REACHED();
    }
