...
// LUI
class LoadUnsignedImmediate : public UJTypeInstruction {
public:
    virtual ~LoadUnsignedImmediate() = default;
    ...
    bool operator==(LoadUnsignedImmediate const&) const = default;

    LoadUnsignedImmediate(i32 immediate, Register rd)
...
// AUIPC
class AddUnsignedImmediateToProgramCounter : public UJTypeInstruction {
public:
    virtual ~AddUnsignedImmediateToProgramCounter() = default;
    virtual String to_string(DisplayStyle display_style, u32 origin, Optional<SymbolProvider const&> symbol_provider) const override;
    virtual String mnemonic() const override;
    virtual bool instruction_equals(InstructionImpl const&) const override;
    bool operator==(AddUnsignedImmediateToProgramCounter const&) const = default;

    AddUnsignedImmediateToProgramCounter(i32 immediate, Register rd)
        : UJTypeInstruction(immediate, rd)
    {
    }
};
...
NonnullOwnPtr<LoadUnsignedImmediate> parse_lui(u32 instruction);
NonnullOwnPtr<JumpAndLink> parse_jal(u32 instruction);
NonnullOwnPtr<JumpAndLinkRegister> parse_jalr(u32 instruction);
NonnullOwnPtr<AddUnsignedImmediateToProgramCounter> parse_auipc(u32 instruction);
