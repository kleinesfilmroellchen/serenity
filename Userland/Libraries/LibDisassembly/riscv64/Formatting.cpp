...
String LoadUnsignedImmediate::to_string(DisplayStyle display_style, u32, Optional<SymbolProvider const&>) const
{
    return MUST(String::formatted("{:10} {}, {:d}", mnemonic(), format_register(destination_register(), display_style), immediate()));
}

String LoadUnsignedImmediate::mnemonic() const
{
    return "lui"_string;
}
...
String JumpAndLink::to_string(DisplayStyle display_style, u32 origin, Optional<SymbolProvider const&> symbol_provider) const
{
    ...
    if (display_style.use_aliases == DisplayStyle::UseAliases::Yes && destination_register() == RegisterABINames::zero)
...
String AddUnsignedImmediateToProgramCounter::to_string(DisplayStyle display_style, u32, Optional<SymbolProvider const&>) const
{
    return MUST(String::formatted("{:10} {}, {}", mnemonic(), format_register(destination_register(), display_style), immediate()));
}

String AddUnsignedImmediateToProgramCounter::mnemonic() const
{
...
{
    if (display_style.use_aliases == DisplayStyle::UseAliases::Yes && rounding_mode() == RoundingMode::DYN)
        return {};
