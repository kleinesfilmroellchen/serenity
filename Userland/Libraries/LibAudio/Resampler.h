...
// Zero-order hold resample (just copy and drop samples as needed),
// which is the lowest quality resample possible, but very fast.
template<typename SampleType>
class ZeroOrderHoldResampler {
public:
    ZeroOrderHoldResampler(u32 source, u32 target)
        : m_source(source)
        , m_target(target)
    {
        VERIFY(source > 0);
