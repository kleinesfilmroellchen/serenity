...
{
#define oom_warning "FLAC Warning: Couldn't use sample hint to reserve {} bytes padding; ignoring hint."

    auto const samples_per_seekpoint = m_sample_rate * seekpoint_period_seconds;
    auto const seekpoint_count = round_to<size_t>(static_cast<double>(sample_count) / samples_per_seekpoint);
    // Will always be divisible by 4, which means we will never have stray padding that's not divisible by 4.
    ...
    if (!m_cached_metadata_blocks.is_empty() && m_cached_metadata_blocks.last().type == FlacMetadataBlockType::PADDING) {
        auto result = m_cached_metadata_blocks.last().data.try_resize(seektable_size);
...
ErrorOr<void> FlacWriter::write_metadata_block(FlacRawMetadataBlock& block)
{
    if (m_state == WriteState::FormatFinalized) {
        ...
        // Other code should make sure that this never happens; e.g. the seektable only has sizes divisible by 4 anyways.
...
ErrorOr<void> FlacWriter::write_frame()
{
    ...
    auto const frame_start_offset = TRY(write_frame_for(subframe_samples, channel_type));
    ...
    if (!last_seekpoint.has_value() || static_cast<double>(sample_index - last_seekpoint->sample_index) >= seekpoint_period_samples) {
        dbgln_if(FLAC_ENCODER_DEBUG, "Inserting seekpoint at sample index {} frame start {}", sample_index, frame_start_offset);
