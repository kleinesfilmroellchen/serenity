...
public:
    ...
    // Add a time zone to this timestamp.
    // The new timestamp will represent the same point in time within that time zone.
    ZonedDateTime with_time_zone(TimeZone::TimeZone) const;
    ZonedDateTime with_current_time_zone() const;
    ...
    explicit LocalDateTime(Core::DateTime const& legacy_date_time)
        : LocalDateTime(UnixDateTime::from_unix_timespec({ .tv_sec = legacy_date_time.timestamp(), .tv_nsec = 0 }))
    {
    }
