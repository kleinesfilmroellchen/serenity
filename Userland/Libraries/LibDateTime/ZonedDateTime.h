...
// This time class uses the UTC standard and Unix timestamps, i.e. it accounts for leap seconds.
// Around positive leap seconds, this class’s behavior is currently unknown and possibly not POSIX-conforming.
// As of 2025, there have been 27 positive leap seconds (no negative leap seconds so far),
// so this date time value is expected to be 27 seconds ahead of atomic time.
// FIXME: Add a variant of this that uses the TAI standard (atomic time), i.e. that has no leap seconds.
class ZonedDateTime {
public:
    ...
    // This is the offset from 1970-01-01T00:00:00+00:00 in UTC, accounting for leap seconds.
...
private:
    ...
    // - This value contains leap seconds, with repeated (non-monotonic) time stamps for leap seconds.
