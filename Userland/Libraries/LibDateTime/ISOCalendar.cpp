...
static ISOCalendar::OutputParts to_parts_impl(Duration offset)
{
    ...
    parts.month = month;
    auto ak_weekday = day_of_week(year, parts.month, parts.day_of_month);
    parts.weekday = ak_weekday == 0 ? ISOCalendar::Weekday::Sunday : static_cast<ISOCalendar::Weekday>(ak_weekday);
...
ISOCalendar::OutputParts ISOCalendar::to_parts(ZonedDateTime const& date_time)
{
    ...
    //        We would want to determine whether we’re currently in a leap second and if so, modify the output to show seconds == 60.
...
static ErrorOr<i64> timestamp_from_parts(ISOCalendar::InputParts const& input_parts)
{
    ...
    // FIXME: This is legal with leap second smearing.
