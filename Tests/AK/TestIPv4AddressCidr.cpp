...
TEST_CASE(sanity_check)
{
    ...
    IPv4AddressCidr address = address_result.value();
...
{
    IPv4AddressCidr address = IPv4AddressCidr::create(IPv4Address(192, 0, 2, 1), 24).value();
...
{
    IPv4AddressCidr address = IPv4AddressCidr::create(IPv4Address(192, 0, 2, 1), 8).value();
    EXPECT_EQ(address.ip_address(), IPv4Address(192, 0, 2, 1));
...
{
    IPv4AddressCidr address = IPv4AddressCidr::create(IPv4Address(192, 0, 2, 1), 32).value();
    EXPECT_EQ(address.length(), (u32)32);
...
{
    IPv4AddressCidr address = IPv4AddressCidr::create(IPv4Address(0, 0, 0, 0), 0).value();
    EXPECT(address.contains(IPv4Address(192, 0, 2, 1)));
}
...
TEST_CASE(should_parse_cidr_string)
{
    ...
    EXPECT_EQ(address.value(), IPv4AddressCidr::create(IPv4Address(192, 0, 2, 1), 24).value());
}
