...
ErrorOr<size_t> TCPSocket::protocol_send(UserOrKernelBuffer const& data, size_t data_length)
{
    ...
    if (ip_version() == IPVersion::IPv4)
        routing_decision = route_to(peer_address().get<IPv4Address>(), local_address().get<IPv4Address>(), adapter);
...
ErrorOr<void> TCPSocket::send_tcp_packet(u16 flags, UserOrKernelBuffer const* payload, size_t payload_size, RoutingDecision* user_routing_decision)
{
    ...
    auto adapter = bound_interface().with([](auto& bound_device) -> RefPtr<NetworkAdapter> { return bound_device; });
    RoutingDecision routing_decision;
    routing_decision = user_routing_decision ? *user_routing_decision : route_to(peer_address().get<IPv4Address>(), local_address().get<IPv4Address>(), adapter);
    if (routing_decision.is_zero())
        return set_so_error(EHOSTUNREACH);
    ...
    routing_decision.adapter->fill_in_ipv4_header(*packet, local_address().get<IPv4Address>(),
        routing_decision.next_hop, peer_address().get<IPv4Address>(), TransportProtocol::TCP,
    ...
    tcp_packet.set_checksum(compute_tcp_checksum(local_address().get<IPv4Address>(), peer_address().get<IPv4Address>(), tcp_packet, payload_size));
...
ErrorOr<void> TCPSocket::protocol_bind()
{
    ...
    TRY(m_adapter.with([this](auto& adapter) -> ErrorOr<void> {
        if (has_specific_local_address() && !adapter) {
            adapter = NetworkingManagement::the().from_ipv4_address(local_address().get<IPv4Address>());
            if (!adapter)
    ...
    if (local_port() == 0) {
        ...
        return sockets_by_tuple().with_exclusive([&](auto& table) -> ErrorOr<void> {
            ...
            while (true) {
                IPv4SocketTuple proposed_tuple(local_address().get<IPv4Address>(), port, peer_address().get<IPv4Address>(), peer_port());
...
ErrorOr<void> TCPSocket::protocol_connect(OpenFileDescription& description)
{
    ...
    auto routing_decision = route_to(peer_address().get<IPv4Address>(), local_address().get<IPv4Address>());
    if (routing_decision.is_zero())
        return set_so_error(EHOSTUNREACH);
...
void TCPSocket::retransmit_packets()
{
    ...
    auto adapter = bound_interface().with([](auto& bound_device) -> RefPtr<NetworkAdapter> { return bound_device; });
    auto routing_decision = route_to(peer_address().get<IPv4Address>(), local_address().get<IPv4Address>(), adapter);
    if (routing_decision.is_zero())
        return;
    ...
    m_unacked_packets.with_exclusive([&](auto& unacked_packets) {
        for (auto& packet : unacked_packets.packets) {
            ...
            routing_decision.adapter->fill_in_ipv4_header(*packet.buffer,
                local_address().get<IPv4Address>(), routing_decision.next_hop, peer_address().get<IPv4Address>(),
