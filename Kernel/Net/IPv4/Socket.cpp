...
ErrorOr<NonnullRefPtr<Socket>> IPSocket::create(int domain, int type, int protocol)
{
    ...
    if (type == SOCK_RAW) {
        auto raw_socket = adopt_ref_if_nonnull(new (nothrow) IPSocket(domain, type, protocol, move(delegate), move(receive_buffer), {}));
        if (raw_socket)
...
IPSocket::IPSocket(int domain, int type, int protocol, NonnullOwnPtr<IPSocketDelegate> delegate, NonnullOwnPtr<DoubleBuffer> receive_buffer, OwnPtr<KBuffer> optional_scratch_buffer)
...
bool IPSocket::is_in_multicast_group(IPAddress multicast_address) const
{
    ...
    case IPVersion::IPv6:
        TODO();
    case IPVersion::IPv4:
        return static_cast<IPv4Socket const&>(*m_delegate).multicast_memberships().contains_slow(multicast_address.get<IPv4Address>());
    }
}
...
{
    if (!address.has<IPv4Address>())
        return ENOTSUP;
    m_local_address = address.get<IPv4Address>();
    return {};
}
...
ErrorOr<void> IPv4Socket::set_peer_address(IPAddress address)
{
    if (!address.has<IPv4Address>())
        return ENOTSUP;
    m_peer_address = address.get<IPv4Address>();
    dbgln("set peer address {}", m_peer_address);
    return {};
}
...
ErrorOr<void> IPSocket::connect(Credentials const&, OpenFileDescription& description, Userspace<sockaddr const*> address, socklen_t address_size)
{
    ...
    dbgln("setting peer {} : {}", m_delegate->peer_address(), m_peer_port);
...
ErrorOr<size_t> IPSocket::sendto(OpenFileDescription&, UserOrKernelBuffer const& data, size_t data_length, [[maybe_unused]] int flags, Userspace<sockaddr const*> addr, socklen_t addr_length)
{
    ...
    if (ip_version() == IPVersion::IPv4) {
        auto routing_decision = route_to(peer_address().get<IPv4Address>(), local_address().get<IPv4Address>(), adapter, allow_broadcast, allow_using_gateway);
