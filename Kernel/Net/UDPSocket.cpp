...
UDPSocket::UDPSocket(int protocol, NonnullOwnPtr<IPSocketDelegate> delegate, NonnullOwnPtr<DoubleBuffer> receive_buffer)
    : IPSocket(delegate->domain(), SOCK_DGRAM, protocol, move(delegate), move(receive_buffer), {})
{
}
...
ErrorOr<size_t> UDPSocket::protocol_send(UserOrKernelBuffer const& data, size_t data_length)
{
    ...
    // TODO: support IPv4
    auto routing_decision = route_to(peer_address().get<IPv4Address>(), local_address().get<IPv4Address>(), adapter, allow_broadcast);
    ...
    routing_decision.adapter->fill_in_ipv4_header(*packet, local_address().get<IPv4Address>(), routing_decision.next_hop,
        peer_address().get<IPv4Address>(), TransportProtocol::UDP, udp_buffer_size, type_of_service(), ttl());
